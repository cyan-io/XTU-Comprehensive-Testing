...
def trans(df: pd.DataFrame):
    return df.replace('不及格', 50.).replace('及格', 60.).replace('中', 70.).replace('良', 80.).replace('优', 90.) \
        .fillna(NOT_CHOOSE).sort_index()


df1: pd.DataFrame = pd.read_excel('成绩表/湘潭大学班级成绩表 .xls', skiprows=3, index_col=0)
df1 = trans(df1)
cred1 = [4., 2., 2., 1., 3.5, 1., 4., 1.]

df2: pd.DataFrame = pd.read_excel('成绩表/湘潭大学班级成绩表  (1).xls', skiprows=3, index_col=0)
df2 = trans(df2)
...
res.to_csv('加权均分.csv', encoding='GBK')
